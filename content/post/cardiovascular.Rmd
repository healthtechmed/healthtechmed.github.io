...
Nós sabemos que se a variável `sex` for igual a 0, o paciente é mulher e se for igual a `1`, ele é homem. Mas o R não sabe disso, então ele não vai conseguir usar as palavras "homem" e "mulher" na hora de montar os gráficos.

É muito fácil resolver isso utilizando os [Factors](https://www.datamentor.io/r-programming/factor/) do R.
...
Para entender melhor a distribuição de doenças cardiovasculares em relação ao sexo, vamos plotar um gráfico parecido com o de cima, mas agora nós vamos colorir a barra de cada sexo com a quantidade de controles e pacientes com doenças cardiacas daquele sexo

```{r}
ggplot(heart, aes(x = sex)) + 
  geom_bar(aes(y = ..count.. / sum(..count..), fill = target)) +
  labs(x = "Sexo", y = "Quantidade (%)") +
  scale_fill_discrete(name = "Diagnóstico", labels = c("Controle", "Doença Cardíaca")) +
  ggtitle("Distribuição de Doença Cardíaca pelo Sexo")
```

A grande diferença do código desse gráfico está na segunda linha, onde nós dizemos ao R como renderizar as barras: 
```r
geom_bar(aes(y = ..count.. / sum(..count..), fill = target)) +
```

Diferentemente do outro gráfico, aqui nós especificamos que o preenchimento de cada barra (`fill`) deve ser dado pela variável `target`. Por fim, a linha que chama a função `scale_fill_discrete` diz ao R o texto da legenda.

No gráfico, é possível observar que o número de pacientes com doença cardíaca está balanceado entre os dois sexos (o tamanho das barras azuis são parecidos), então é possível fazer comparações robustas entre os sexos dentro desse subgrupo.
